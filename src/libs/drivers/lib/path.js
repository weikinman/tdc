...
export function filename(path, includeDir = false) {
	if (!path) throw new Error('Path is empty');
...
export function safeFileExtension(e, maxLength = null) {
...
export function toFileProtocolPath(filePathEncode, os = null) {
...
export function toSystemSlashes(path, os = null) {
	if (os === null) os = process.platform;
...
export function rtrimSlashes(path) {
	return path.replace(/[\/\\]+$/, '');
}

export function ltrimSlashes(path) {
	return path.replace(/^\/+/, '');
}
...
export const isUncPath = (path, os = null) => {
...
export function extractExecutablePath(cmd) {
